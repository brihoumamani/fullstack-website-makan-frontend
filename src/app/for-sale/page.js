...
const algerianLocations = [
  { commune: 'Hydra', wilaya: 'Alger', address: 'Boulevard du 11 Décembre' },
  { commune: 'Sidi Yahia', wilaya: 'Alger', address: 'Rue Sidi Yahia' },
  { commune: 'El Biar', wilaya: 'Alger', address: 'Avenue Ali Khodja' },
  { commune: 'Chéraga', wilaya: 'Alger', address: 'Route de Bouchaoui' },
  { commune: 'Kouba', wilaya: 'Alger', address: 'Avenue de la Liberté' },
  { commune: 'Akid Lotfi', wilaya: 'Oran', address: 'Boulevard Akid Lotfi' },
  { commune: 'Canastel', wilaya: 'Oran', address: 'Front de Mer Canastel' },
  { commune: 'Bir El Djir', wilaya: 'Oran', address: 'Boulevard Millenium' },
  { commune: 'Ali Mendjeli', wilaya: 'Constantine', address: 'Nouvelle Ville UV 05' },
  { commune: 'Sidi Mabrouk', wilaya: 'Constantine', address: 'Boulevard Principal' },
  { commune: 'Sidi Aissa', wilaya: 'Annaba', address: 'Corniche Sidi Aissa' },
  { commune: 'Seraïdi', wilaya: 'Annaba', address: 'Route du Mont de l’Edough' }
];

const propertyTypes = ['apartment', 'villa', 'duplex', 'commercial'];

const initialSaleProperties = Array.from({ length: 20 }, (_, index) => {
  const loc = algerianLocations[index % algerianLocations.length];
  const type = propertyTypes[index % propertyTypes.length];
  const priceBase = type === 'villa' ? 65000000 : type === 'duplex' ? 45000000 : type === 'commercial' ? 35000000 : 25000000;
  const price = priceBase + (index * 1500000);
  const sqm = type === 'villa' ? 280 + (index * 10) : type === 'duplex' ? 190 : type === 'commercial' ? 85 : 110 + (index * 5);
  const roomsCount = type === 'villa' ? 5 : type === 'duplex' ? 4 : type === 'commercial' ? 1 : 3;

  return {
    _id: `sale-${index + 1}`,
    title: type === 'villa' ? `Belle Villa avec Jardin à ${loc.commune}` : type === 'duplex' ? `Superbe Duplex F4 à ${loc.commune}` : type === 'commercial' ? `Local Commercial Spacieux à ${loc.commune}` : `Appartement Haut Standing F${roomsCount} à ${loc.commune}`,
    price: price,
    rentOrSale: 'sale',
    type: type,
    address: loc.address,
...
export default function ForSalePage() {
  ...
  const [propertiesList, setPropertiesList] = useState(initialSaleProperties);

  useEffect(() => {
    async function loadProperties() {
      try {
        const res = await fetch(`${API_BASE}/api/properties?rentOrSale=sale`);
        if (res.ok) {
          const data = await res.json();
          if (data.data && Array.isArray(data.data) && data.data.length > 0) {
            const apiItems = data.data;
            const fallbackFiltered = initialSaleProperties.filter(
  ...
  const filteredProperties = useMemo(() => {
    let result = propertiesList.filter((property) => {
      ...
      if (priceRange !== 'all') {
        if (priceRange === '0-20m' && property.price >= 20000000) return false;
        if (priceRange === '20m-40m' && (property.price < 20000000 || property.price > 40000000)) return false;
        if (priceRange === '40m+' && property.price <= 40000000) return false;
      }
  ...
  return (
    <div className="w-full bg-[#FAFAFA] dark:bg-[#0f172a] min-h-screen py-8 transition-colors duration-200">
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 space-y-7">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5">
            ...
            <div className="relative min-w-[120px] sm:min-w-[130px] flex-1 sm:flex-initial">
              <select
                ...
                className="w-full appearance-none bg-white dark:bg-[#141720] border border-gray-300 dark:border-slate-700 rounded-md py-2.5 pl-3.5 pr-8 text-xs sm:text-sm text-gray-800 dark:text-gray-100 outline-none focus:border-[#6366F1] dark:focus:border-[#818CF8] cursor-pointer transition-colors font-normal"
              >
                <option value="all" className="dark:bg-[#141720]">Price (DZD)</option>
                <option value="0-20m" className="dark:bg-[#141720]">&lt; 20.000.000 DA</option>
                <option value="20m-40m" className="dark:bg-[#141720]">20M - 40M DA</option>
                <option value="40m+" className="dark:bg-[#141720]">&gt; 40.000.000 DA</option>
              </select>
              <div className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-700 dark:text-gray-300">
                <svg className="w-4 h-4 stroke-[2.5]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        ...
        {/* 3. 20 Property Cards Grid (5 rows of 4 columns) */}
